...
# BLF log file configuration
BLF_LOG_FILE = "can_data_log.blf"

# Setup BLF writer
blf_writer = can.BLFWriter(open(BLF_LOG_FILE, "wb"))

# Function to write a CAN-like message to the BLF log
def write_to_blf(packet):
    try:
        # Map the data packet fields to CAN message data
        message = Message(
            arbitration_id=0x123,  # Example CAN ID
            data=[
                int(packet["speed_kmh"] // 1),  # Speed in km/h (integer part)
                int((packet["speed_kmh"] % 1) * 100),  # Speed decimal as a percentage
                int(packet["location"]["x"] % 256),  # Location X (lower byte)
                int(packet["location"]["y"] % 256),  # Location Y (lower byte)
                int(packet["control"]["throttle"] * 100),  # Throttle percentage
                int(packet["control"]["brake"] * 100),  # Brake percentage
                int(packet["control"]["steering"] * 100),  # Steering angle
                packet["control"]["gear"],  # Gear position
            ],
            is_extended_id=False,
        )
        blf_writer.write(message)
    except Exception as e:
        print(f"Error writing to BLF: {e}")
...
def start_carla():
    try:
        ...
        while time.time() - start_time < duration:
            ...
            # Write CAN-like data to BLF log
            write_to_blf(data_packet)
