...
Run from Python scripts/:
    ...
    python experiments/compare_carla.py ../Data/carla/processed/new --max-windows 10000 --latency-n 500
"""
...
ROOT = _SCRIPTS  # Python scripts/ (historical name used below)
_CKPT = _HERE / "checkpoints" / "carla"
_FIG = _HERE / "figures" / "carla"

# Writable matplotlib cache (sandbox / CI friendly).
_MPLDIR = _FIG / ".mplconfig"
...
def eval_mvp(trace, ts, args, rng) -> dict:
    """Load GRU checkpoint from experiments/ or /tmp/fed_ids_checkpoints."""
    from models.gru import GRUAutoencoder

    name = args.mvp_name
    candidates = [_CKPT, Path("/tmp/fed_ids_checkpoints")]
    bundle = next(
        (c for c in candidates if (c / f"{name}.npz").exists()),
        None,
    )
    if bundle is None:
        sys.exit(
            f"MVP checkpoint not found under {_CKPT} or /tmp/fed_ids_checkpoints.\n"
            f"Train first:  python -m experiments.train --dataset carla --model gru "
            f"--name {name}"
        )
    ...
    scaler = np.load(bundle / f"{name}_scaler.npz")
...
def main():
    ...
    ap.add_argument(
        ...
        default=_SCRIPTS.parent / "Data" / "carla" / "processed" / "new",
        help="parquet file or directory (default: Data/carla/processed/new)",
    )
