...
# ── Two-broker setup ─────────────────────────────────────────────────────────
# Federation MQTT broker — receives outgoing weights, sends global model back.
# This is the federated-learning aggregation server.
DEFAULT_FED_BROKER       = "192.168.0.125"
DEFAULT_FED_PORT         = 1883

# Telemetry MQTT broker — receives live vehicle telemetry + anomaly state.
# This is the Flask dashboard MQTT broker.
DEFAULT_TELEMETRY_BROKER = "192.168.0.89"
DEFAULT_TELEMETRY_PORT   = 1883

# Telemetry publish throttle. Main loop ticks every 50 ms (~20 Hz). With
# throttle=100, dashboard sees one update every 5 s — smooth without
# flooding the broker, and easy on Pi CPU.
TELEMETRY_PUB_INTERVAL   = 100
...
class FederatedClient:
    def __init__(self, client_id: str,
                 fed_broker: str = DEFAULT_FED_BROKER,
                 fed_port: int   = DEFAULT_FED_PORT,
                 telemetry_broker: str = DEFAULT_TELEMETRY_BROKER,
                 telemetry_port: int   = DEFAULT_TELEMETRY_PORT,
                 can_interface: str    = 'can0'):
        self.client_id = client_id

        # Two brokers: federation (weights) and telemetry (dashboard).
        self.fed_broker       = fed_broker
        ...
        # Federation MQTT client and state
        self.mqtt              = None
        self.connected         = False

        # Telemetry MQTT client and state (separate, never share a broker)
        self.mqtt_telemetry      = None
    ...
    def connect(self):
        """Open connections to both brokers. Federation broker handles
        weight exchange with the central server; telemetry broker handles
        live dashboard updates and anomaly state."""

        # ---- Federation broker (192.168.0.125 by default) -------------------
        try:
        ...
        self.mqtt.on_connect    = self._on_connect_fed
        self.mqtt.on_message    = self._on_message_fed
        self.mqtt.on_disconnect = self._on_disconnect_fed
        print(f"[Client] Connecting to FED broker "
              f"{self.fed_broker}:{self.fed_port}...")
        self.mqtt.connect(self.fed_broker, self.fed_port, keepalive=120)
        self.mqtt.loop_start()

        # ---- Telemetry broker (192.168.0.89 by default) ---------------------
        try:
            self.mqtt_telemetry = mqtt.Client(
                client_id=f"tel_{self.client_id}",
                callback_api_version=mqtt.CallbackAPIVersion.VERSION2)
        except Exception:
            self.mqtt_telemetry = mqtt.Client(client_id=f"tel_{self.client_id}")

        self.mqtt_telemetry.on_connect    = self._on_connect_telemetry
        self.mqtt_telemetry.on_disconnect = self._on_disconnect_telemetry
        # Note: no on_message — telemetry broker is publish-only from us.
        print(f"[Client] Connecting to TEL broker "
              f"{self.telemetry_broker}:{self.telemetry_port}...")
        self.mqtt_telemetry.connect(self.telemetry_broker,
                                     self.telemetry_port, keepalive=120)
        self.mqtt_telemetry.loop_start()

    # ---- Federation broker callbacks ----------------------------------------

    def _on_connect_fed(self, client, userdata, flags, rc, properties=None):
        if hasattr(rc, 'value'):
            rc = rc.value
        if rc == 0:
            self.connected = True
            print(f"[Client {self.client_id}] FED broker connected")
    ...
    def _on_disconnect_fed(self, client, userdata, disconnect_flags=None,
                           rc=None, properties=None):
        self.connected = False
        print(f"[Client {self.client_id}] FED broker disconnected — "
              f"will reconnect")

    def _on_message_fed(self, client, userdata, message):
        try:
            ...
            print(f"[Client] FED message error: {e}")

    # ---- Telemetry broker callbacks -----------------------------------------
    ...
    def _on_connect_telemetry(self, client, userdata, flags, rc,
        ...
        if rc == 0:
            ...
            print(f"[Client {self.client_id}] TEL broker connected")
            self._publish_status("online")
        else:
            print(f"[Client] TEL MQTT connect failed: rc={rc}")

    def _on_disconnect_telemetry(self, client, userdata,
                                  disconnect_flags=None, rc=None,
                                  properties=None):
        self.telemetry_connected = False
        print(f"[Client {self.client_id}] TEL broker disconnected — "
              f"will reconnect")

    # ---- Status publish (now on telemetry broker) ---------------------------

    def _publish_status(self, status: str):
        """Status updates feed the dashboard, not the federation server."""
        if not self.telemetry_connected:
            return
        topic = f"vehicle/{self.client_id}/status"
        self.mqtt_telemetry.publish(topic, json.dumps({
    ...
    # ---- Live telemetry publish (new) ---------------------------------------

    def _publish_telemetry(self, score=None, is_anomaly=False):
        """Publish a snapshot of current telemetry + anomaly state in the
        exact shape flask_server_dashboard_v16.py expects. Throttled by
        TELEMETRY_PUB_INTERVAL ticks so the dashboard isn't flooded."""
        self._telemetry_tick += 1
        if self._telemetry_tick % TELEMETRY_PUB_INTERVAL != 0:
            return
        if not self.telemetry_connected:
            return
        if len(self.collector.buffer) == 0:
            return

        # Latest raw frame (un-normalised — the dashboard wants real units).
        # Buffer layout matches DataCollector.FEATURE_NAMES =
        #   [speed_kmh, battery_level, throttle, brake,
        #    steering, gear, location_x, location_y]
        frame = self.collector.buffer[-1]

        payload = {
            "client_id": self.client_id,
            "timestamp": time.time(),
            "values": {
                "Speed (km/h)":      float(frame[0]),
                "Battery Level (%)": float(frame[1]),
                "Throttle":          float(frame[2]),
                "Brake":              float(frame[3]),
                "Steering":           float(frame[4]),
                "Gear":               float(frame[5]),
            },
            "Location": {
                "x": float(frame[6]),
                "y": float(frame[7]),
                "z": 0.0,                       # z is dropped during ingest
            },
            "anomaly": {
                "detected":  bool(is_anomaly),
                "score":     float(score) if score is not None else 0.0,
                "threshold": float(self.model.threshold)
                              if self.model.calibrated else 0.0,
            },
        }
        topic = f"vehicle/{self.client_id}/telemetry"
        self.mqtt_telemetry.publish(topic, json.dumps(payload))
    ...
    def _detect_anomaly(self):
        ...
        if window is None:
            # Still publish telemetry from the latest raw frame so the
            # dashboard sees the vehicle alive while the buffer warms up.
            self._publish_telemetry(score=None, is_anomaly=False)
            return
        ...
        if self.model.calibrated:
            ...
            if is_anomaly:
                ...
                # Alert goes to the dashboard broker, not the federation server.
                if self.telemetry_connected:
                    self.mqtt_telemetry.publish(
                        f"vehicle/{self.client_id}/alerts/attack",
                        json.dumps({
                            "client_id": self.client_id,
                            "score":     float(score),
                            "threshold": float(thr),
        ...
        # Always publish telemetry (throttled internally by tick count).
        # If not calibrated yet, score is still informative for the
        # dashboard chart; the 'detected' flag stays False.
        self._publish_telemetry(score=score, is_anomaly=is_anomaly)
    ...
    def run(self):
        ...
        print(f"  FED Broker    : {self.fed_broker}:{self.fed_port}")
        print(f"  TEL Broker    : {self.telemetry_broker}:{self.telemetry_port}")
        ...
        try:
            ...
            if self.mqtt:
                ...
                try:
                    ...
                    pass
            if self.mqtt_telemetry:
                self.mqtt_telemetry.loop_stop()
                try:
...
def main():
    ...
    p.add_argument("--fed-broker",       default=DEFAULT_FED_BROKER,
                   help=f"federation MQTT broker "
                        f"(default {DEFAULT_FED_BROKER})")
    p.add_argument("--fed-port",         type=int, default=DEFAULT_FED_PORT)
    p.add_argument("--telemetry-broker", default=DEFAULT_TELEMETRY_BROKER,
                   help=f"dashboard MQTT broker "
                        f"(default {DEFAULT_TELEMETRY_BROKER})")
    ...
    FederatedClient(
        ...
        fed_broker       = args.fed_broker,
        fed_port         = args.fed_port,
        telemetry_broker = args.telemetry_broker,
