...
class FederatedClient:
    ...
    def __init__(self, client_id: str, broker: str,
        ...
        # Extract edge number from client_id (e.g. "edge_2" → 2)
        # Defaults to 1 if the id doesn't end with a digit.
        try:
            self._edge_num = int(client_id.split('_')[-1])
        except (ValueError, IndexError):
            self._edge_num = 1
            print(f"[Client] Could not parse edge number from '{client_id}' "
                  f"— defaulting to {self._edge_num}")

        self.collector = DataCollector(can_interface, edge_num=self._edge_num)
  ...
  # REPLACE the entire _apply_global_model method (lines 855–871) with:
    ...
    def run(self):
        ...
        _ids = ', '.join(f'0x{k:03X}' for k in self.collector.can_id_map)
        print(f"  CAN IDs       : {_ids}  (edge {self._edge_num})")
