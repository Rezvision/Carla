# flask_server_dashboard_v15.py
#
# Changes from v14:
#   • Top-level vehicle dropdown (edge_1 / edge_2 / edge_3) — every chart,
#     status indicator and control button now operates on the selected vehicle.
#   • All state (DATA_PACKET, ACCUMULATED_DATA, DATA_BUFFER, FIRST_DATA_TIME,
#     LAST_SAVE_TIME) is now a defaultdict keyed by client_id.
#   • MQTT subscribes to wildcard `vehicle/+/telemetry` and routes by topic.
#   • Window-level anomaly only: packet carries one {detected, score, threshold}
#     block instead of per-CAN-signal `attacked` flags. CSV writes per-row
#     Anomaly / AnomalyScore / Threshold columns.
#   • Emergency control now per-vehicle, with a separate "stop ALL" button and
#     a "resume" command published to `vehicle/{client_id}/control`.
#   • New anomaly-score-vs-threshold time chart.
#   • CSV files written under logs/<date>/<client_id>/ to keep vehicles apart.
...
TOPIC_CTRL_FMT       = "vehicle/{client_id}/control"
...
def save_to_csv(client_id, data, start_time):
    ...
    try:
        ...
        fieldnames = [
            ...
            "Location_y",
            "Location_z",
            "Anomaly",          # window-level: yes/no
            "AnomalyScore",     # reconstruction error
            "Threshold",        # IDS threshold at the time
        ]

        # debug dump alongside CSV — easy to diff if a row looks wrong
        ...
        with open(csv_file, 'w', newline='') as f:
            ...
            for packet in data:
                ...
                # flat values dict from the edge client
...
def on_mqtt_message(client, userdata, message):
    try:
        ...
        if client_id not in VEHICLES:
            # accept the packet but flag it — a new edge node may be coming online
            print(f"[MQTT] Unknown vehicle '{client_id}' on {message.topic} — accepting")

        # always stamp with dashboard reception time
...
app.layout = html.Div([
    html.H1("Federated IDS — TCU Monitoring Dashboard (v15)"),
    ...
    html.Div([
        ...
        html.Div([
            html.H4("Anomaly Status (window-level)"),
    ...
    # ── Anomaly score chart (new in v15) ─────────────────────────────────────
    html.Div([
    ...
    # ── Emergency controls ───────────────────────────────────────────────────
    html.Div([
        html.Button("Emergency Stop (selected vehicle)", id="emergency-btn",
                    n_clicks=0,
                    style={'background-color': '#ff4444', 'color': 'white',
                           'margin-right': '10px', 'padding': '10px 15px'}),
        html.Button("Emergency Stop ALL vehicles", id="emergency-all-btn",
                    n_clicks=0,
                    style={'background-color': '#cc0000', 'color': 'white',
                           'margin-right': '10px', 'padding': '10px 15px'}),
        html.Button("Resume (selected vehicle)", id="resume-btn",
                    n_clicks=0,
                    style={'background-color': '#44aa44', 'color': 'white',
                           'padding': '10px 15px'}),
        html.Div(id="status-msg", style={'margin-top': '10px'})
...
def update_anomaly_status(n, vehicle):
    ...
    thr      = anomaly.get("threshold")

    if detected:
        out = [html.P("⚠️ ANOMALY DETECTED!", style={'color': 'red',
                                                    'font-weight': 'bold'})]
        if isinstance(score, (int, float)):
            out.append(html.P(f"Score: {score:.6f}", style={'color': 'red'}))
        if isinstance(thr, (int, float)):
            out.append(html.P(f"Threshold: {thr:.6f}", style={'color': 'red'}))
        return out

    out = [html.P("Normal", style={'color': 'green', 'font-weight': 'bold'})]
    if isinstance(score, (int, float)) and isinstance(thr, (int, float)) and thr > 0:
        ratio = score / thr
        out.append(html.P(f"Score: {score:.6f}  ({ratio*100:.0f}% of threshold)"))
    return out
...
def update_anomaly_chart(n, vehicle):
    ...
    for packet in ACCUMULATED_DATA[vehicle][-300:]:    # last 300 windows
...
@app.callback(
    ...
    [Input("emergency-btn",     "n_clicks"),
     Input("emergency-all-btn", "n_clicks"),
     Input("resume-btn",        "n_clicks")],
    State("vehicle-dropdown", "value"),
    prevent_initial_call=True
)
def handle_control(stop_n, stop_all_n, resume_n, vehicle):
    ctx = dash.callback_context
    if not ctx.triggered:
        return ""
    trigger_id = ctx.triggered[0]["prop_id"].split(".")[0]

    if trigger_id == "emergency-btn":
        if not vehicle:
            return html.P("No vehicle selected", style={"color": "red"})
        topic = TOPIC_CTRL_FMT.format(client_id=vehicle)
        mqtt_client.publish(topic, "emergency_stop")
        return html.P(f"⛔ Emergency stop sent to {vehicle}",
                      style={"color": "red", "font-weight": "bold"})

    if trigger_id == "emergency-all-btn":
        for v in VEHICLES:
            mqtt_client.publish(TOPIC_CTRL_FMT.format(client_id=v),
                                "emergency_stop")
        return html.P(f"⛔ Emergency stop sent to ALL vehicles ({len(VEHICLES)})",
                      style={"color": "red", "font-weight": "bold"})

    if trigger_id == "resume-btn":
        if not vehicle:
            return html.P("No vehicle selected", style={"color": "red"})
        topic = TOPIC_CTRL_FMT.format(client_id=vehicle)
        mqtt_client.publish(topic, "resume")
        return html.P(f"▶ Resume sent to {vehicle}",
